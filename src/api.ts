...
async function findEpisodes(filters: EpisodeFilters) {
...
function buildQuery(filters: EpisodeFilters) {
...
function normalizeParam(param: unknown): string[] | undefined {
  // ensures parameters are consistent (all in an array)
  if (!param) {
    return undefined;
  }
  return Array.isArray(param) ? param as string[] : [String(param)];
}
